...
public class ViewManager {
    ...
    public static boolean handlePostView(HttpServletRequest req, HttpServletResponse resp, String postId) {
        ...
        String cookieName = "viewedPost_" + postId + "_" + userId;  // 쿠키 이름에 사용자 ID 추가
        ...
        if (viewedToday.isEmpty()) {
            ...
            // 쿠키를 생성하여 해당 사용자가 오늘 게시물을 조회했다고 기록
            CookieManager.makeCookie(resp, cookieName, "true", 24 * 60 * 60); // 유효기간 1일 설정
            return true; // 조회수 증가 성공
        } else {
            // 이미 조회한 경우
            return false; // 조회수 증가 안 됨
        }
    }
}
