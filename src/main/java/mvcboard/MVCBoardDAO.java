...
public class MVCBoardDAO extends DBConnPool {
	...
	//기본생성자를 통해 부모클래스의 기본생성자 호출(생략 가능)
	public MVCBoardDAO() {
		super();
	}
	...
	//게시판 목록에 추력할 레코드를 인출하기 위한 메서드 정의
	public List<MVCBoardDTO> selectList(Map<String,Object> map){
		...
		//게시판은 항상 최근 작성한 게시물이 상단에 노출되어야 한다.
		...
		try {
			...
			while(rs.next()) {
				...
				/*
				ResultSet 인스턴스에서 데이터를 추출할때 멤버변수의 타입에 따라
	...
	public int insertWriter(MVCBoardDTO dto) {
		int result = 0;
		try {
			String query = 
	...
	public void updateVisitCount(String idx) {
		// visitcount 컬럼은 number타입이므로 산술연산이 가능
		// 1읠 더한 결과를 컬럼에 재반영하는 형식으로 update 쿼리문 사용.
		...
		try {
			...
			executeUpdate() : insert,update, delete계열의 쿼리문을 실행한다.
			 	반환타입은 int
			만약 쿼리 실행후 별도의 반환값이 필요하지 않다면 위 2개의 메서드 중
			어떤 것을 사용해도 무방하다.
			*/
			//psmt.executeQuery();
